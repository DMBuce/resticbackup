...
Run each C<*.pre> hook. See L</HOOKS> for details.

=item *

Look up C<$RESTICBACKUP_FOO> in the config. See L</CONFIGURATION> for details.
...
Run each C<*.post> hook. See L</HOOKS> for details.
...
C<CHECKMK_INTEGRITY_SPOOL=/path>: The path to the spool file
...
C<CHECKMK_SNAPSHOTS_SPOOL=/path>: The path to the spool file

=item *

C<CHECKMK_SNAPSHOTS_THRESHOLDS=period:num_crit:num_warn>: The period of time to check, the warning threshold, and te critical threshold (e.g. C<1day:6:10>)
